...
class Generator:
    ...
    def codegen(self, node, builder):
        ...
        if isinstance(node, lz_ast.Constant):
            ...
            if node.dType == float:
                ...
                fmt_ptr = builder.gep(self.constants[node.value], [ir.IntType(32)(0), ir.IntType(32)(0)],
                                      inbounds=False, name=node.value + "_ptr")

                return fmt_ptr

        elif isinstance(node, lz_ast.Variable):
        ...
        elif isinstance(node, lz_ast.Array_Declaration):

            self.variables[(node.name, self.scope)] = node.dType

            r = node.elements

            if isinstance(r, lz_ast.Variable) or isinstance(r, lz_ast.Array_Element):
        ...
        elif isinstance(node, lz_ast.Array_Element):
            ...
            if isinstance(node.index, lz_ast.Variable) or isinstance(node.index, lz_ast.Array_Element):
        ...
        elif isinstance(node, lz_ast.Assignment):
            ...
            if node.dType == float:
                ...
                if isinstance(r, lz_ast.Constant):
        ...
        elif isinstance(node, lz_ast.BinaryOp):

            cmp_op = {">", "<", "!=", ">=", '<=', "=="}

            l, r = node.children()

            if isinstance(l, lz_ast.Variable) or isinstance(l, lz_ast.Array_Element):
                lvalue = self.codegen(l, builder)
        ...
        elif isinstance(node, lz_ast.UnaryOp):
        ...
        elif isinstance(node, lz_ast.Output):

            raw_data = node.children()

            data = self.codegen(raw_data, builder)

            if isinstance(raw_data, lz_ast.Array_Element):
                data = builder.load(data, name=raw_data.name + "_val", align=None)

            if raw_data.dType == float:
                fmt_ptr = builder.gep(self.double_fmt, [ir.IntType(32)(0), ir.IntType(32)(0)], inbounds=False,
                                      name="fmt_ptr")

                if isinstance(raw_data, lz_ast.Variable):
                    data = builder.load(data, name=raw_data.name + "_val", align=None)

            elif raw_data.dType == str:
                fmt_ptr = builder.gep(self.string_fmt, [ir.IntType(32)(0), ir.IntType(32)(0)], inbounds=False,
                                      name="fmt_ptr")

            elif raw_data.dType == int:
                fmt_ptr = builder.gep(self.int_fmt, [ir.IntType(32)(0), ir.IntType(32)(0)], inbounds=False,
                                      name="fmt_ptr")
            ...
            fmt_ptr = builder.gep(self.newline_fmt, [ir.IntType(32)(0), ir.IntType(32)(0)], inbounds=False,
                                  name="fmt_ptr")
            builder.call(self.printf, [fmt_ptr], name="print")

            return builder

        elif isinstance(node, lz_ast.If):
        ...
        elif isinstance(node, lz_ast.While):
        ...
        elif isinstance(node, lz_ast.Input):
        ...
        elif isinstance(node, lz_ast.Function_Decl):
        ...
        elif isinstance(node, lz_ast.Function_Call):
            ...
            for arg in node.args:
                ...
                if isinstance(arg, lz_ast.Array_Element) or isinstance(arg, lz_ast.Variable):
        ...
        elif isinstance(node, lz_ast.Return):

            res = self.codegen(node.data, builder)

            if isinstance(node.data, lz_ast.Array_Element) or isinstance(node.data, lz_ast.Variable):
...
if __name__ == "__main__":
    ast = LZ_Parser(LZ_Lexer).parse("x = 2 + 2")

...
'''
A modular parser which builds an AST using lz_ast's AST node
...
class LZ_Parser:
    ...
    def __init__(self, lexer=LZ_Lexer):
    ...
    def p_array_decl_stmt(self, p):
        ...
        p[0] = lz_ast.Array_Declaration(dType, name, elements)
    ...
    def p_assignment_stmt(self, p):
        ...
        var = lz_ast.Variable(dType, var, length)
        p[0] = lz_ast.Assignment("=", dType, var, expr)

    def p_array_assign_stmt(self, p):
        '''assignment_stmt : array_index EQUALS expression'''

        if (p[1].name, self.scope) in self.variable_types:
            p[0] = lz_ast.Assignment("=", self.variable_types[(p[1].name, self.scope)], p[1], p[3])
        else:
    ...
    def p_input_stmt(self, p):
        ...
        if isinstance(p[2], lz_ast.Array_Element):
            name = p[2].name
        else:
            name = p[2]
        ...
        if isinstance(p[2], lz_ast.Array_Element):
            ...
            var = lz_ast.Variable(self.variable_types[(name, self.scope)], p[2], 0)

        p[0] = lz_ast.Input(var, self.variable_types[(name, self.scope)])

    def p_output_stmt(self, p):
        '''output_stmt : OUTPUT expression'''

        p[0] = lz_ast.Output(p[2])

    def p_return_stmt(self, p):
        '''return_stmt : RETURN expression'''

        p[0] = lz_ast.Return(p[2])
    ...
    def p_function_stmt(self, p):
        ...
        p[0] = lz_ast.Function_Decl(name, args, p[3], dType)
    ...
    def p_expression_arithmetic_binop(self, p):
        ...
        if p[1].dType == None or p[3].dType == None:
            ...
            p[0] = lz_ast.BinaryOp(p[2], p[1], p[3], float, 0)

        elif p[1].dType == int and p[3].dType == int:
            p[0] = lz_ast.BinaryOp(p[2], p[1], p[3], int, 0)
    ...
    def p_expression_unop(self, p):
        ...
        p[0] = lz_ast.UnaryOp(p[2].dType, p[1], p[2], p[2].length)
    ...
    def p_expression_func_call(self, p):
        ...
        if len(p) == 5:
            ...
            if type(p[3]) == list:
                p[0] = lz_ast.Function_Call(p[1], p[3], self.functions[p[1]], 0)

            else:
                p[0] = lz_ast.Function_Call(p[1], [p[3]], self.functions[p[1]], 0)

        elif len(p) == 4:
            p[0] = lz_ast.Function_Call(p[1], [], self.functions[p[1]], 0)
    ...
    def p_expression_array_val(self, p):
        ...
        if (p[1], self.scope) in self.variable_types:
            p[0] = lz_ast.Array_Element(self.variable_types[(p[1], self.scope)], p[1], p[3], 0)
        else:
            p[0] = lz_ast.Array_Element(None, p[1], p[3], None)
    ...
    def p_expression_var(self, p):
        ...
        if (p[1], self.scope) in self.variable_types:
            ...
            p[0] = lz_ast.Variable(self.variable_types[(p[1], self.scope)], p[1], length)

        else:
            p[0] = lz_ast.Variable(None, p[1], 0)
